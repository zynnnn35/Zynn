...
const { default: WAConnection, useMultiFileAuthState, Browsers, DisconnectReason, makeInMemoryStore, makeCacheableSignalKeyStore, fetchLatestBaileysVersion, proto, getAggregateVotesInPollMessage } = require('baileys-zynn');
...
const { reconnectJadibots } = require('./src/jadibot'); // Import fungsi reconnect
	...
	naze.ev.on('connection.update', async (update) => {
		...
		if ((connection == 'connecting' || !!qr) && pairingCode && !naze.authState.creds.registered && !pairingStarted) {
			...
			setTimeout(async () => {
				...
				let code = await naze.requestPairingCode(phoneNumber, 'rubybott');
